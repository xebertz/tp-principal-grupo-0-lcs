...
`base_model = tf.keras.applications.Variante(...)` crea la base del modelo utilizando una arquitectura preentrenada llamada "Variante". Esto es donde especificarías el nombre de la arquitectura específica que se desea utilizar (por ejemplo, EfficientNetB0, ResNet50, MobileNetV2, etc.). La opción `include_top=False` indica que se omitirán las capas totalmente conectadas (top) del modelo preentrenado, ya que se agregarán capas personalizadas para adaptar el modelo a una tarea específica. También se especifica el tamaño de entrada de las imágenes con `input_shape=(224, 224, 3)` (imágenes de 224x224 píxeles, con 3 canales de color).
...
model = Model(inputs=base_model.input, outputs=output): Con esta línea, se crea el modelo completo que toma las imágenes como entrada y produce las salidas de clasificación. Especifica las entradas (base_model.input) y las salidas (output) del modelo.
